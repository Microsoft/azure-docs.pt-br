...
O Microsoft Azure StorSimple Virtual Array é uma solução de armazenamento integrado que gerencia  armazenamento entre um  virtual array local executando em um hipervisor e o armazenamento em nuvem do Microsoft Azure. O Virtual Array é um servidor de arquivos eficiente, econômico e facilmente gerenciável, ou solução de servidor iSCSI, que elimina vários problemas e despesas associados à proteção de dados e ao armazenamento em empresas. O Virtual Array é particularmente adequado para o armazenamento de dados de arquivamento acessados com pouca frequência.

Este artigo apresenta uma visão geral do Virtual Array – confira aqui alguns outros recursos:

* Para conhecer as melhores práticas recomendadas, confira [Práticas recomendadas do Virtual Array do StorSimple](storsimple-ova-best-practices.md).
...
O Virtual Array suporta o protocolo SMB (Server Message Block) ou iSCSI. Ele é executado em sua infraestrutura existente do hipervisor e proporciona camadas para a nuvem, backup na nuvem, restauração rápida, recuperação em nível de item e recursos de recuperação de desastre.

A tabela a seguir resume os recursos importantes do Virtual Array do StorSimple.

| Recurso | Virtual Array do StorSimple |
| --- | --- |
| Requisitos de instalação |Usa a infraestrutura de virtualização (Hyper-V ou VMware) |
| Disponibilidade |Nó único |
| Capacidade total (incluindo nuvem) |Até 64 TB de capacidade utilizável por virtual array |
| Capacidade local |De 390 GB a 6,4 TB de capacidade utilizável por virtual array (é necessário provisionar 500 GB a 8 TB de espaço em disco) |
...
O StorSimple conecta usuários e servidores ao armazenamento do Azure, em minutos, sem modificações em aplicativos.

A tabela a seguir descreve algumas das principais vantagens oferecidas pela solução Virtual Array do StorSimple.

| Recurso | Benefício |
| --- | --- |
| Integração transparente |O Virtual Array dá suporte ao protocolo SMB ou ao iSCSI. A movimentação de dados entre a camada local e a camada de nuvem é contínua e transparente para o usuário. |
| Redução nos custos de armazenamento |Com o StorSimple, você provisiona armazenamento local suficiente para atender às demandas atuais pelos dados mais usados e de grande interesse. Conforme as necessidades de armazenamento crescem, o StorSimple distribui dados de menor interesse em camadas para  armazenamento em nuvem com melhor custo/benefício. Os dados duplicados são removidos e comprimidos antes de serem enviados à nuvem para reduzir ainda mais os custos e os requisitos de armazenamento. |
...
| Mobilidade de dados |Os dados carregados na nuvem podem ser acessados de outros locais para fins de recuperação e migração. Observe que você pode restaurar dados somente para o virtual array original. No entanto, use recursos de recuperação de desastre para restaurar todo o virtual array para outra matriz virtual. |
...
O StorSimple Virtual Array é mais adequado para dados acessados com pouca frequência. Embora o virtual array tenha um cache local para melhorar o desempenho, os usuários devem assumir que o dispositivo atende a arquivos na camada mais baixa de armazenamento (nuvem). Cada virtual array pode gravar e ler no armazenamento do Azure em aproximadamente 100 Mbps. Esse link é compartilhado entre todas as solicitações que chegam ao dispositivo e pode tornar-se um gargalo, conforme mostra o diagrama a seguir.

![Arquivamento de nuvem](./media/storsimple-ova-overview/cloud-archiving.png)

Quando vários usuários simultâneos acessam o virtual array, todos eles compartilham a conexão com o Azure, levando a um desempenho inferior. Não há garantia de desempenho por usuário e o dispositivo processa solicitações individuais conforme elas chegam.

O StorSimple Virtual Array não é adequada para cargas de trabalho que exigem alta disponibilidade. O Virtual Array é um dispositivo em um nó único que apresenta indisponibilidade quando as atualizações de software são instaladas. Os administradores devem planejar uma janela de manutenção de 30 minutos de três a quatro vezes por ano.

## <a name="workflows"></a>Fluxos de trabalho

O StorSimple Virtual Array é particularmente adequado para os fluxos de trabalho a seguir:
...
Você pode usar o serviço Gerenciador de Dispositivos StorSimple no Portal do Azure para gerenciar dados armazenados em vários dispositivos e em vários locais. Isso é particularmente útil em cenários de filiais distribuídas. Observe que você deve criar instâncias separadas do serviço Gerenciador de Dispositivos do StorSimple para gerenciar os virtuais arrays e dispositivos físicos StorSimple. Observe também que o virtual array usa o novo Portal do Azure ao invés do Portal clássico do Azure.
...
Com o virtual array, os instantâneos de nuvem fornecem uma cópia pontual de um volume ou compartilhamento, independente de local. Instantâneos de nuvem são habilitados por padrão e não podem ser desabilitados. Todos os volumes e compartilhamentos sofrem backup ao mesmo tempo através de uma única política diária de backup, e você pode obter backups adicionais ad hoc sempre que necessário.

### <a name="data-protection-and-disaster-recovery"></a>Recuperação de desastre e proteção de dados
O virtual array dá suporte aos seguintes cenários de proteção de dados e recuperação de desastre:
...
* **Recuperação de desastres** – Use o recurso failover para recuperar todos os volumes ou compartilhamentos para um novo virtual array. Crie o novo virtual array e registre-a com o serviço Gerenciador de Dispositivos StorSimple, depois realize o failover do virtual array original. O novo  virtual array assumirá os recursos provisionados.

## <a name="storsimple-virtual-array-components"></a>Componentes do Virtual Array do StorSimple

O virtual array inclui os seguintes componentes:
...
O virtual array é uma solução de armazenamento de nó único que oferece armazenamento primário, gerencia a comunicação com o armazenamento em nuvem e ajuda a garantir a segurança e a confidencialidade de todos os dados armazenados no dispositivo.

O virtual array está disponível em um modelo para download. O virtual array tem uma capacidade máxima de 6,4 TB no dispositivo (com um requisito de armazenamento subjacente de 8 TB) e 64 TB incluindo o armazenamento em nuvem.

A matriz virtual tem os seguintes recursos:

* Tem bom custo/benefício. Usa sua infraestrutura de virtualização existente e pode ser implantado em seu hipervisor Hyper-V ou VMware existente.
* Reside no datacenter e pode ser configurado como um servidor iSCSI ou um servidor de arquivos.
* É integrado à nuvem.
...
* Você pode aplicar atualizações ao virtual array, assim como você as aplicaria a um dispositivo físico.

> [!NOTE]
> O virtual array não pode ser expandido. Portanto, é importante provisionar um armazenamento adequado ao criar o virtual array.
...
* Gerenciar, de um único serviço, vários StorSimple Virtual Arrays.
* Configurar e gerenciar configurações de segurança para StorSimple Virtual Arrays. (Criptografia na nuvem é dependente de APIs do Microsoft Azure)
...
Você pode usar o serviço Gerenciador de Dispositivo StorSimple para realizar a administração diária de seu virtual array.
...
O virtual array inclui uma interface de usuário baseada na Web, que é usada para configuração e registro único do dispositivo, com o serviço Gerenciador de Dispositivo StorSimple. Você pode usá-la para desligar e reiniciar o virtual array, executar testes de diagnóstico, atualizar o software, alterar a senha de administrador do dispositivo, exibir logs do sistema e contatar o Suporte da Microsoft para registrar uma solicitação de serviço.
...
> [!NOTE]
> Você pode especificar um volume fixado localmente; neste caso, os dados permanecem no virtual array e nunca são distribuídos em camadas para a nuvem. Para obter mais informações, vá para [Volumes e compartilhamentos fixados localmente](#locally-pinned-shares-and-volumes).


### <a name="locally-pinned-shares-and-volumes"></a>Volumes e compartilhamentos fixados localmente

Você pode criar apropriadamente os compartilhamentos e volumes como fixados localmente. Essa funcionalidade garante que os dados necessários para aplicativos críticos permaneçam na matriz virtual e não sejam distribuídos em camadas na nuvem. Compartilhamentos e volumes fixados localmente têm as seguintes características:
...
> [!NOTE]
> Os dados armazenados no virtual array não passaram por eliminação de duplicação ou compactação. Toda a eliminação de duplicação e compactação ocorrem imediatamente antes do envio dos dados para a nuvem.
